...
export type RiskSharePublicHeaderVariant = "brand" | "monthly" | "prework" | "minimal";
...
export default function RiskSharePublicHeader({
  ...
  return (
    <div className={`${VARIANT_CLASS[variant]} p-5 sm:p-6 ${className ?? ""}`}>
      ...
      <h1 className="rsx-pub-title-lb mt-3 text-2xl font-black leading-tight tracking-tight">{title}</h1>
      {description ? <p className="mt-2 text-sm font-semibold leading-6 opacity-90">{description}</p> : null}
    </div>
  );
}
