...
type Summary = { total: number; passCount: number; failCount: number; mustFailCount: number };
              ...
              <div key={item.id} className="flex items-center justify-between rounded-lg border border-red-100 bg-red-50 p-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {item.우선순위 && <span className="mr-2 font-semibold text-orange-600">{item.우선순위}</span>}
                    {item.기한 && <span>기한: {item.기한}</span>}
                  </p>
                </div>
                <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-bold text-red-700">{item.상태 || "미설정"}</span>
              </div>
            ))}
          </div>
        </div>
