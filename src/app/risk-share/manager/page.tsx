...
import ManagerSidebar from "@/components/risk-share/manager/ManagerSidebar";
import ManagerHeader from "@/components/risk-share/manager/ManagerHeader";
import ManagerPageHead from "@/components/risk-share/manager/ManagerPageHead";
import StatCardsSection from "@/components/risk-share/manager/StatCardsSection";
import MonthlyBanner from "@/components/risk-share/manager/MonthlyBanner";
import AiBriefingCard from "@/components/risk-share/manager/AiBriefingCard";
import FieldReferenceWeatherCard from "@/components/risk-share/manager/FieldReferenceWeatherCard";
import SafetyResourceBoard from "@/components/risk-share/manager/SafetyResourceBoard";
import WeeklyTrendCard from "@/components/risk-share/manager/WeeklyTrendCard";
import ReviewStatusCard from "@/components/risk-share/manager/ReviewStatusCard";
import RecentSubmissionsTable from "@/components/risk-share/manager/RecentSubmissionsTable";
import RepresentativeSignatureCard from "@/components/risk-share/manager/RepresentativeSignatureCard";
import WorkflowTimelineCard from "@/components/risk-share/manager/WorkflowTimelineCard";
...
export default async function RiskShareManagerHomePage({ searchParams }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#F4F6FB] text-slate-950 lg:flex">
      <ManagerSidebar
        companyLabel={companyLabel}
        managerHref={managerHref}
        fieldHref={fieldHref}
        monthlyHref={monthlyHref}
        monthlyConfirmationCount={monthlyConfirmationCount}
        preworkConfirmationCount={preworkConfirmationCount}
      ...
      <section className="min-w-0 flex-1">
        ...
        <div className="mx-auto flex w-full max-w-[1220px] flex-col gap-5 px-4 py-5 lg:px-7">
          <ManagerPageHead
            title="위험성평가 공유확인 현황"
            description="현장 QR로 들어온 확인·의견 흐름을 한 화면에서 봅니다. 상단 5개 카드가 이번 달의 핵심입니다."
          ...
          <StatCardsSection
            ...
            preworkConfirmationCount={preworkConfirmationCount}
            anonymousFeedbackCount={anonymousFeedbackCount}
            visitorConfirmationCount={visitorConfirmationCount}
            representativeTotalCount={representativeTotalCount}
          />
          ...
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
            <AiBriefingCard
              ...
              generatedAtLabel={getNowKstTimeLabel()}
            />
            <FieldReferenceWeatherCard
              status={fieldReferenceWeather.status}
              headline={fieldReferenceWeather.headline}
          ...
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
            <WeeklyTrendCard />
            <ReviewStatusCard monthLabel={monthLabel} signatureNotSubmittedCount={signatureNotSubmittedCount} />
          </div>

          <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
            <RecentSubmissionsTable />
            <div className="flex flex-col gap-5">
              <RepresentativeSignatureCard
                totalCount={representativeTotalCount}
                signatureConfirmedCount={signatureConfirmedCount}
                signatureNotSubmittedCount={signatureNotSubmittedCount}
              />
          ...
          <SafetyResourceBoard
            koshaTags={KOSHA_HAZARD_REFERENCE_TAGS}
            koshaLink={KOSHA_OFFICIAL_LINK_URL}
            newsItems={fieldReferenceSafetyNews}
            newsMoreLink={SAFETY_NEWS_MORE_LINK_URL}
          />
        </div>
      </section>
    </main>
