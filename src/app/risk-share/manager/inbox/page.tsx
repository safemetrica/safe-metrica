...
export default async function ManagerInboxPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const auditEvents = detail?.type === "monthly"
    ? await listManagerInboxAuditEvents(tenant.tenant.code, detail.id).catch(() => [])
    : [];
    ...
    <section className="mx-auto max-w-6xl">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]">
        ...
        <aside className="rounded-2xl bg-white p-5">
          {detail ? <><span className="text-xs font-black text-emerald-700">{TYPE_LABEL[detail.type]}</span><h2 className="mt-2 text-xl font-black">{detail.title}</h2><dl className="mt-5 space-y-3 text-sm"><div><dt className="font-bold text-slate-500">접수 일시</dt><dd>{formatSeoulCustomerDateTime(detail.createdAt)}</dd></div><div><dt className="font-bold text-slate-500">제출자</dt><dd>{detail.submitterLabel}</dd></div>{detail.location ? <div><dt className="font-bold text-slate-500">장소</dt><dd>{detail.location}</dd></div> : null}<div><dt className="font-bold text-slate-500">내용</dt><dd className="whitespace-pre-wrap">{detail.content || "작성된 내용이 없습니다."}</dd></div><div><dt className="font-bold text-slate-500">처리 상태</dt><dd>{STATUS_LABEL[detail.status]}</dd></div>{detail.actionNote ? <div><dt className="font-bold text-slate-500">조치 메모</dt><dd className="whitespace-pre-wrap">{detail.actionNote}</dd></div> : null}</dl>{detail.type === "monthly" ? <section className="mt-5 border-t pt-5"><h3 className="text-sm font-black">처리 이력</h3>{auditEvents.length ? <ol className="mt-3 space-y-3">{auditEvents.map((event) => <li key={event.id} className="rounded-xl bg-slate-50 p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{STATUS_LABEL[event.fromStatus]} → {STATUS_LABEL[event.toStatus]}</strong><span className="text-slate-500">{formatSeoulCustomerDateTime(event.createdAt)}</span></div>{event.actionNote ? <p className="mt-2 whitespace-pre-wrap text-slate-700">{event.actionNote}</p> : null}</li>)}</ol> : <p className="mt-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-500">아직 기록된 상태 변경이 없습니다.</p>}</section> : null}{detail.canTransition ? <a href={`${managerHref}#confirmation-review`} className="mt-5 block rounded-xl bg-slate-950 px-4 py-3 text-center text-sm font-black text-white">검토·처리 계속</a> : <p className="mt-5 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900">이 유형은 현재 상세 확인만 제공합니다. 상태 변경은 검증된 업무별 계약이 연결된 뒤 제공됩니다.</p>}</> : <p className="text-sm text-slate-500">목록에서 접수를 선택해 주세요.</p>}
        </aside>
      </div>
    </section>
  </main>;
