...
export default function WorkerDemoClient() {
  ...
  const [resumedAtCompletion, setResumedAtCompletion] = useState(false);
  ...
  useEffect(() => {
    window.setTimeout(() => {
      ...
      setResumedAtCompletion(savedState.workerReportSubmitted);
    }, 0);
  }, []);
      ...
      <div className="mx-auto w-full max-w-[430px] pb-24">
        ...
        <section className="mt-5 rounded-[1.75rem] border border-slate-700 bg-slate-900 p-5 shadow-xl">
          ...
          {step === 3 && (
            <div>
              {resumedAtCompletion && (
                <div className="mb-4 rounded-2xl border border-sky-400/30 bg-sky-950/30 p-4 text-center">
                  <p className="text-sm font-black text-sky-100">이전 체험 기록이 남아 있습니다.</p>
                  <p className="mt-1 text-xs font-bold leading-5 text-sky-100/70">처음부터 다시 체험하거나 현장관리자 흐름으로 이어갈 수 있습니다.</p>
                </div>
              )}
        ...
        <div className="sticky bottom-0 z-20 mt-5 border-t border-slate-800 bg-slate-950/95 pb-3 pt-3 backdrop-blur">
          ...
          {step === 3 && resumedAtCompletion && (
            <button type="button" onClick={restartWorkerDemo} className="mb-3 flex min-h-12 w-full items-center justify-center rounded-2xl border border-emerald-400/40 bg-slate-900 px-5 py-3 text-sm font-black text-emerald-100 transition-all active:scale-[0.98]">
              처음부터 다시 체험
            </button>
          )}
