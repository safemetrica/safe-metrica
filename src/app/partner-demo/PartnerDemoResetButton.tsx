...
export default function PartnerDemoResetButton() {
  ...
  return (
    <div className="mt-4 rounded-2xl border border-slate-700/80 bg-slate-950/45 p-3">
      <div className="flex items-center justify-between gap-3">
        <p className="text-xs font-bold leading-5 text-slate-400">반복 체험용 상태 관리</p>
        <button
          type="button"
          onClick={resetDemoState}
          className="min-h-11 shrink-0 rounded-xl border border-slate-600 bg-slate-900 px-4 py-2 text-xs font-black text-slate-300 shadow-sm transition-all duration-150 hover:border-slate-500 hover:bg-slate-800 active:scale-[0.98] active:translate-y-0.5"
        >
          체험 초기화
        </button>
      </div>
