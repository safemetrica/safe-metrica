...
export default async function FieldParticipationPage({ searchParams }: PageProps) {
  ...
  const copy = getFieldParticipationCopy(params.company);
  const companyCode = copy.code;
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-5 text-slate-900">
      <div className="mx-auto max-w-3xl">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black text-blue-700">{copy.badge}</p>
        ...
        >
          ...
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <h2 className="text-sm font-black text-amber-800">{copy.formNoticeTitle}</h2>
