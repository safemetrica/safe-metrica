...
const INTERNAL_MANAGER_LINKS = [
  {
    ...
    description: "공유확인, 현장 의견, 근로자대표 참여확인, Export, 월간요약을 확인합니다.",
  },
  {
    title: "공유팩 월간 운영요약",
  ...
  },
  {
    ...
    description: "근로자대표 확인 linkId 발급, 제출 현황, 폐기·만료 상태를 관리합니다.",
  },
];
  ...
  return (
    <section className="rounded-3xl border border-emerald-500/25 bg-slate-900/85 p-5 shadow-xl shadow-slate-950/30">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
            QR 배포와 고객 안내 전에 현재 업체 코드가 포함된 링크인지 확인합니다.
            현장 근로자용 링크에는 반드시 company query가 포함되어야 합니다.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        {INTERNAL_MANAGER_LINKS.map((item) => (
          <article
            key={item.path}
      ...
      <div className="mt-5 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm leading-6 text-amber-100">
        근로자대표 참여확인 제출 링크는 고정 링크가 아니라 관리 화면에서 확인 범위와 만료일을 지정해
        linkId 방식으로 발급합니다. 고객별 안내문에는 실제 고객 데이터, 내부 인증정보, 내부 원장 주소를 포함하지 않습니다.
      </div>
    </section>
  );
}
