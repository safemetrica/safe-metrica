...
export function evaluateActionEvidence(input: {
  ...
  const reason =
    status === "양호"
      ? (ruleReason ?? "현재 작업 수준에 필요한 기본 증빙이 확인됩니다.")
      : status === "보완 추천"
        ? (ruleReason ?? "TBM 실시 증빙은 확인되지만, 작업명 또는 조치상태 기준으로 작업 대상·조치 사진을 남기면 조치 이력 신뢰도가 더 높아지는 항목입니다.")
  ...
  const suggestion =
    status === "양호"
      ...
      : status === "보완 추천"
        ? `${expectedText}을(를) 추가하면 조치 이력 신뢰도가 향상됩니다. 체조나 TBM 활동사진만으로는 작업·조치 증빙이 충분하다고 보지 않습니다.`
