...
const privilegeMigration = read("supabase/migrations/20260721060000_lock_confirmation_review_event_privileges.sql");
...
const checks = [
  ...
  ["immutable audit events", /create table if not exists public\.risk_share_confirmation_review_events/.test(migration)
    && /revoke all privileges[\s\S]*risk_share_confirmation_review_events[\s\S]*from public, anon, authenticated, service_role/.test(privilegeMigration)
    && /grant select[\s\S]*risk_share_confirmation_review_events[\s\S]*to service_role/.test(privilegeMigration)
    && !/\bgrant\s+[^;]*\b(insert|update|delete)\b[^;]*\bon\s+(table\s+)?public\.risk_share_confirmation_review_events/i.test(privilegeMigration)],
  ["audit privilege migration fails closed", /table owner or RLS mismatch/.test(privilegeMigration)
    && /RPC contract mismatch/.test(privilegeMigration)
    && /unexpected client policy/.test(privilegeMigration)
    && /unexpected table grant/.test(privilegeMigration)
    && /service_role ACL mismatch/.test(privilegeMigration)],
