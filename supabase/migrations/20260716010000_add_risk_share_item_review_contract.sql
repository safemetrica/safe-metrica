...
--   3. review_risk_share_item -- atomic, SECURITY DEFINER RPC. Re-derives
--      actor role/status/company from tenant_membership inside the
--      transaction; never trusts a client-supplied role. Locks the item row
--      FOR UPDATE before checking lock state, idempotency, and revision, so
--      concurrent/duplicate submissions for the same item serialize on that
...
comment on column public.risk_share_item_review_events.request_payload is
  'Normalized client-submitted review fields only (task_name/hazard/current_controls/improvement_plan/risk_level/worker_share_summary/worker_visible + action). No raw source row, private Blob pathname/URL, checksum, token, email, phone number, or signature original.';
...
begin
  ...
  -- 5.1 Membership verification. Role/status/company are read from the
  -- membership row itself; p_company_code is only the query key.
  ...
  -- 5.2 Item row lock. A mismatched company_code resolves the same as a
  ...
  -- 5.3 Locked item block, checked inside the row lock (not a prior
  ...
  v_normalized_payload := jsonb_build_object(
    ...
    'workerVisible', p_worker_visible
  );

  -- 5.4 Idempotency check, evaluated before the stale-revision check (5.5)
  -- so a network retry of an already-successful request replays instead of
  -- failing stale. Because the item row is already locked FOR UPDATE above,
  -- two concurrent submissions for the same item serialize here rather than
  -- racing on the idempotency_key lookup.
  ...
  where risk_share_item_review_events.company_code = v_company_code
    and risk_share_item_review_events.idempotency_key = p_idempotency_key;
  ...
  begin
    -- 5.8 Audit insert in the same transaction. If this fails for any
    -- reason (including a idempotency_key race that slips past the 5.4
    -- lookup), the item update above rolls back too.
